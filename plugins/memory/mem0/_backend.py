...
class SelfHostedBackend(Mem0Backend):
    ...
    def _json(self, method: str, path: str, **kwargs) -> Any:
        ...
        try:
            ...
            # Log full response body on HTTP errors to aid diagnosis
            # (e.g. 422 validation errors from the mem0 server).
            _logger.warning(
                "Mem0 HTTP %s %s → %s body=%s",
    ...
    def search(self, query: str, *, filters: dict, top_k: int = 10, rerank: bool = False) -> list[dict]:
        # Self-hosted mem0 supports rerank + depth when configured.
        body: dict[str, Any] = {"query": query, "top_k": top_k}
        if rerank:
            body["rerank"] = True
            body["depth"] = "full"  # rerank only applies in full-depth mode
        if filters:
...
class OSSBackend(Mem0Backend):
    ...
    def search(self, query: str, *, filters: dict, top_k: int = 10, rerank: bool = False) -> list[dict]:
        kwargs: dict[str, Any] = {"query": query, "filters": filters, "top_k": top_k}
        if rerank:
            kwargs["rerank"] = True
            kwargs["depth"] = "full"
        response = self._memory.search(**kwargs)
        return _unwrap_results(response)
