window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const FIELDS = [
      ...
      { key: "sliceThreshold", type: "number", min: 200, max: 200000 },
      { key: "slicePieceChars", type: "number", min: 200, max: 200000 },
      ...
      { key: "maxBucketAgeMs", type: "number", min: 60000, max: 7200000 },
    ...
    const zh = {
      ...
      "field.sliceThreshold": "超长消息切片阈值（字符）",
      "hint.sliceThreshold": "单条 user/assistant 消息超过该长度按段落切成多条全量保留（服务端分块按消息粒度、单条不拆会撑爆 context_window），默认 8000",
      "field.slicePieceChars": "切片单片上限（字符）",
      "hint.slicePieceChars": "每片不超过该长度（段落优先→行→硬切，不拆码点/不拆 [REDACTED:] 标记），默认 2000",
      ...
      "field.maxBucketAgeMs": "潮浪桶存活上限（毫秒）",
      "hint.maxBucketAgeMs": "服务端明确拒绝（HTTP 4xx/5xx 校验类）或超时的桶超过该龄即丢弃并计数，原文仍在 dsh 会话日志可回捞；纯连接级失败（宕机）永不按龄丢，默认 1800000（30 分钟）",
    ...
    const GROUPS = [
      ...
      { titleKey: "group.sync", keys: ["syncEnabled", "redactEnabled", "coalesceEnabled", "coalesceIdleMs", "coalesceWindowMs", "coalesceMaxTurns", "coalesceMaxChars", "fastpathChars", "sliceThreshold", "slicePieceChars", "feedbackEnabled"] },
      { titleKey: "group.reliability", keys: ["queueMaxLen", "maxBucketAgeMs", "breakerThreshold", "breakerCooldownMs", "requestTimeoutMs"] },
